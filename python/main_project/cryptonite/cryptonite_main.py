...
from . import cryptonite_parser
from .general_tools import read_file, yes_no, write_to_file, ask_overwrite, input_file
import os
...
def encrypt_file(filename: str = "", key: bytes = bytes()) -> bytes:
    ...
    filename = filename or input_file(ask_overwrite=True, file_exists_required=False)
...
def ask_key(ask_generate_key: bool = False) -> bytes:
    ...
    if yes_no("Load key from file?") :
        filename = input_file(ask_overwrite=False, file_exists_required=True)
...
def shellcode_c_crypter(shellcode_filename: str = "", key: bytes = bytes(), platform: str = "", output_filename = "crypted.c") -> str:
    ...
    shellcode_filename = shellcode_filename or input_file(ask_overwrite=False, file_exists_required=True)
...
def compile_c_file_to_executable(filename: str = "", platform: str = "", compiler: str = "", compiler_options: str = "", output_filename: str = "out", static: bool = True) -> bool:
    """Compile the generated C file to an executable."""
    platform = platform.lower()
    if not filename:
        filename = input_file(prompt_text="output", file_exists_required=False)
    if platform == "windows":
    ...
    if not compiler:
        compiler = input("Compiler executable (gcc): ")
    
    command = f"{compiler} {filename} -o {output_filename}{'.exe' if platform == 'windows' else ''} -lsodium -mwindows {"-static" if static else ""} {compiler_options}"
